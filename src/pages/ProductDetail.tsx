...
import { Star, ShieldCheck, Truck, RotateCcw, RotateCw, Heart, ShoppingBag, ShoppingCart, Plus, Minus, Check, Users, Loader2, Building2, Percent, Camera, X, ArrowLeft, ArrowUp, Zap, Clock, Sparkles, MessageSquare, PenLine, Info, Lock, Share2, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Play, QrCode, MapPin, FileText, Sliders, HelpCircle, ThumbsUp, Search, Navigation, Compass, Store } from 'lucide-react';
    ...
    <div id="product-detail-root" className={`relative w-full font-['Inter',sans-serif] text-xs pb-16 transition-colors duration-200 overflow-visible ${
      ...
      <div className={`px-4 sm:px-6 lg:px-8 py-3 space-y-2 border-b relative z-20 transition-colors ${
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <button
            type="button"
            id="product-detail-back-to-top-btn"
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-xs active:scale-95 border cursor-pointer ${
              isLightMode
                ? 'bg-white hover:bg-neutral-50 text-neutral-800 border-[#EADFC9] hover:border-[#F5A623] hover:text-[#0F1A3C]'
                : 'bg-[#0B1B32] hover:bg-[#122644] text-neutral-200 border-[#162744] hover:border-[#F5A623] hover:text-[#F5A623]'
            }`}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                >
                  <Heart className={`w-3.5 h-3.5 transition-transform ${isWishlisted ? 'fill-red-500 text-red-500 animate-heart-pop' : 'text-current'}`} />
                </button>
              </div>
            </div>
